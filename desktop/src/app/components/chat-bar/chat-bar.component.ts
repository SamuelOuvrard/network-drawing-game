...
export class ChatBarComponent implements OnInit, OnDestroy {
  ...
  openExternalWindow(): void {
    this.socketService.bind("openExternalChatCallback", (data: any) => {
      ...
      this.socketService.emit("openExternalChat", {
        linkedSocketId: data.externalWindowSocketid,
      });
    });
    localStorage.setItem(
      "joinedChats",
      JSON.stringify(this.chatService.joinedChatList)
    );
    localStorage.setItem(
      "notJoinedChats",
      JSON.stringify(this.chatService.notJoinedChatList)
    );
    ...
    this.gameService.chatWindow.loadURL(
      "file://" + __dirname + "/index.html#/chat"
    );
    ...
    this.gameService.chatWindow.on("close", () => {
      this.socketService.emit("closeExternalChat", {
        mainWindowId: this.socketService.socketId,
      });
      localStorage.removeItem("joinedChats");
  ...
  createChat(): void {
    if (
      this.createChatForm.value.chatName == "" ||
      !this.createChatForm.value.chatName
    )
      return;
    if (this.createChatForm.valid) {
      ...
      this.snackBar.open(
        "Le nom du canal doit être en 1 et 12 charactères!",
        "",
        {
          duration: 2000,
        }
